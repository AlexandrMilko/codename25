...
import math
import numpy as np
from disruptor.green_screen.find_similar.VanishingPoint.line import Line


# Threshold by which lines will be rejected wrt the horizontal
REJECT_DEGREE_TH = 4.0 # Default was 4.0
...
def GetLines(Image):
    ...
    EdgeImage = cv2.Canny(GrayImage, 40, 255)

    # Finding Lines in the image
    Lines = cv2.HoughLinesP(EdgeImage, 1, np.pi / 180, 10, None, 20, 15)
    ...
    if Lines is None:
        ...
        before_avg_img = np.zeros((1024, 1024, 3), dtype=np.uint8)
        for line in Lines:
            coords = line[0]
            cv2.line(before_avg_img, [int(coords[0]), int(coords[1])], [int(coords[2]), int(coords[3])], (0, 0, 255))

        new_line_img = np.zeros((1024, 1024, 3), dtype=np.uint8)
        cast_lines = []
        for line in Lines:
            coords = line[0]
            try:
                cast_lines.append(Line(coords))
            except Exception as e:
                print(e, "SKIPPING the line")
        avg_lines = Line.get_averaged_lines(cast_lines)
        for line in avg_lines:
            coords = line.get_coords()
            cv2.line(new_line_img, [int(coords[0]), int(coords[1])], [int(coords[2]), int(coords[3])], (0,0,255))
        print(len(Lines), "number of lines before")
        print(len(avg_lines), "number of lines after")
        # cv2.imshow("Before averaging", before_avg_img)
        # cv2.imshow("My averaged lines", new_line_img)
        # cv2.waitKey(0)
        cv2.destroyAllWindows()

    # Filtering Lines wrt angle
    # FilteredLines = FilterLines(Lines)
    avg_Lines = [line.convert_into_right_format() for line in avg_lines]
    FilteredLines = FilterLines(avg_Lines)
