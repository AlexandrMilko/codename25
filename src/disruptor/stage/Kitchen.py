...
from disruptor.stage.FurniturePiece import FurniturePiece, Bed, Curtain, Plant, KitchenTableWithChairs
...
class Kitchen(Room):
    ...
    def stage(self, current_user_id):
        ...
        random_index = random.randint(0, len(pixels_for_placing) - 1)
        render_parameters = (kitchen_table_with_chairs.calculate_rendering_parameters(self, pixels_for_placing[random_index], yaw_angle, (roll_rad, pitch_rad), current_user_id))
        width, height = get_image_size(self.original_image_path)
        render_parameters['resolution_x'] = width
        render_parameters['resolution_y'] = height
        bed_image = kitchen_table_with_chairs.request_blender_render(render_parameters)
        background_image = Image.open(prerequisite_path)
