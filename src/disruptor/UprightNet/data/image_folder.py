...
class InferenceFolder(data.Dataset):
    ...
    def load_imgs(self, img_path, normal_path):
    ...
    def resize_imgs(self, infer_data, resized_width, resized_height):
        infer_data['img'] = cv2.resize(infer_data['img'],
                                       (resized_width, resized_height),
                                       interpolation=cv2.INTER_AREA)
        infer_data['cam_normal'] = cv2.resize(infer_data['cam_normal'],
                                       (resized_width, resized_height),
                                       interpolation=cv2.INTER_NEAREST)
        infer_data['global_normal'] = cv2.resize(infer_data['global_normal'],
                                       (resized_width, resized_height),
                                       interpolation=cv2.INTER_NEAREST)
        infer_data['mask'] = cv2.resize(infer_data['mask'],
                                       (resized_width, resized_height),
                                       interpolation=cv2.INTER_NEAREST)

        return infer_data

    def crop_imgs(self, infer_data, start_x, start_y, crop_w, crop_h):
        infer_data['img'] = infer_data['img'][start_y:start_y+crop_h,
                                              start_x:start_x+crop_w, :]
        infer_data['cam_normal'] = infer_data['cam_normal'][start_y:start_y+crop_h,
                                                            start_x:start_x+crop_w, :]
        infer_data['global_normal'] = infer_data['global_normal'][start_y:start_y+crop_h,
                                                                  start_x:start_x+crop_w, :]
        infer_data['mask'] = infer_data['mask'][start_y:start_y+crop_h,
                                                start_x:start_x+crop_w]
    ...
    def __getitem__(self, index):
        ...
        infer_data = self.load_imgs(img_path, normal_path)

        original_h, original_w = infer_data['img'].shape[0], infer_data['img'].shape[1]
        ...
        if self.is_train:
            ...
            infer_data = self.crop_imgs(infer_data,
                                        start_x, start_y,
                                        crop_w, crop_h)
            infer_data = self.resize_imgs(infer_data,
                                          self.input_width,
                                          self.input_height)
            ...
            infer_data = self.crop_imgs(infer_data, start_x, start_y, crop_w, crop_h)
            infer_data = self.resize_imgs(infer_data, self.input_width, self.input_height)

        ratio_x = float(infer_data['img'].shape[1]) / float(crop_w)
        ratio_y = float(infer_data['img'].shape[0]) / float(crop_h)

        fx = self.fx * ratio_x
        fy = self.fy * ratio_y

        img_1 = np.float32(infer_data['img']) / 255.0
        cam_normal = infer_data['cam_normal']
        # R_g_c = infer_data['R_g_c']

        global_normal = infer_data['global_normal']
        # upright_normal = self.rotate_normal(R_g_c, cam_normal)

        mask = infer_data['mask']
        ...
        #
        #     print('%s from rotation matrix: roll %f, pitch %f, yaw %f' % (
        #     img_path, math.degrees(gt_roll), math.degrees(gt_pitch), math.degrees(gt_yaw)))
        ...
        # targets_1['upright_geo'] = torch.from_numpy(
        #     np.ascontiguousarray(upright_geo).transpose(2, 0, 1)).contiguous().float()
        #
