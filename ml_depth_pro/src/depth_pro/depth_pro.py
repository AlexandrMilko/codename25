...
DEFAULT_MONODEPTH_CONFIG_DICT = DepthProConfig(
    ...
    checkpoint_uri=Path.ML_DEPTH_PRO_CHECKPOINT.value,
...
def visualize_inverse_depth(depth: torch.Tensor):
    """
    Visualizes the inverse depth (1.0 / depth).
    ...
    Returns:
    -------
        Displays the normalized inverse depth using a color map.
    """
    # Compute inverse depth
    inverse_depth = 1.0 / torch.clamp(depth, min=1e-4, max=1e4)

    # Normalize inverse depth to [0, 1] for better visualization
    normalized_inverse_depth = (inverse_depth - inverse_depth.min()) / (inverse_depth.max() - inverse_depth.min())

    # Convert to NumPy for plotting (if using CPU)
    depth_np = normalized_inverse_depth.cpu().numpy()

    # Plot the inverse depth map
    plt.imshow(depth_np, cmap='plasma')  # You can choose another colormap if you prefer
